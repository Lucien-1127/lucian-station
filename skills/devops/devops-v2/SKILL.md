...
- `references/gcp-tools-setup.md` — GCP 工具設定、API 啟用、服務帳號權限
- `references/syncthing-headless-setup.md` — Syncthing 無頭伺服器安裝、設定、故障排除
- `references/config-backup-secrets.md` — 配置備份機敏資料清理流程
- `references/gcp-nested-virt-docker-android.md` — GCP 巢狀虛擬化啟用、Android 模擬器 TYPE-Q 部署、NoVNC/VNC 存取
- `scripts/android-screen-mirror.sh` — 將 Android 模擬器畫面鏡像到 Xvfb（修正 NoVNC 空白問題）
