...
### Interactive (recommended first install)

```bash
hermes memory setup
```
This runs a curses-based picker. Select `honcho` from the list, then follow the prompts:
- Cloud or local? → `cloud` (uses api.honcho.dev) or `local` (self-hosted)
- Paste your API key from https://app.honcho.dev
- Set peer names, recall mode, write frequency

### Manual / non-interactive fallback

When the curses picker cannot run (SSH, agent context, cron, non-TTY):

1. Set `memory.provider: honcho` via CLI:
   ```bash
   hermes config set memory.provider honcho
   ```

2. Create `~/.hermes/honcho.json` with your API key and settings:
   ```json
   {
     "apiKey": "hch-v3-xxxx...",
     "peerName": "your-name",
...
3. Optionally set `HONCHO_API_KEY` in `~/.hermes/.env`.

This bypasses the interactive wizard entirely. Verify with `hermes memory status`.

### Manual / non-interactive fallback

When the curses picker won't run (agent context, SSH, non-TTY), create the config directly:

1. Set `memory.provider: honcho` in `~/.hermes/config.yaml`:
```yaml
memory:
  memory_enabled: true
...
# If `hermes honcho` subcommand is available (plugin CLI registered):
...
Run `hermes memory setup` and select `honcho`. Or use the manual config fallback above. Ensure `memory.provider: honcho` is in `~/.hermes/config.yaml`.

### `hermes honcho` subcommand not found
The `hermes honcho` CLI subcommand depends on the honcho plugin's CLI registration, which may not be loaded in all Hermes versions. Use `hermes memory setup` instead — it discovers Honcho as a provider option and runs the same wizard internally.
...
| Command | Description |
|---------|-------------|
| `hermes memory setup` | **Entry point** — generic memory provider picker; select `honcho` from the list |
| `hermes memory status` | Show active memory provider and config |
| `hermes memory off` | Disable external memory provider |
| *(if `hermes honcho` subcommand available)* | |
