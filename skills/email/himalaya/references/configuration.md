...
**⚠️ Gmail Chinese locale pitfall:** Accounts with zh-TW/zh-CN interface language use
localized folder names. Run `himalaya folder list` to discover the actual names.
Common zh-TW mappings:
- `[Gmail]/寄件備份` instead of `[Gmail]/Sent Mail`
- `[Gmail]/垃圾桶` instead of `[Gmail]/Trash`
- `[Gmail]/草稿` instead of `[Gmail]/Drafts`

Without correct aliases, `himalaya message send` succeeds (SMTP delivers) but saving
to Sent fails with `Folder doesn't exist`, and the command exits non-zero — any
retry logic would duplicate the email.
